...
MDScreen:
        ...
        BoxLayout:
            ...
            ScrollView:
                ...
                MDBoxLayout:
                    ...
                    spacing: "10dp"
                    size_hint_y: None
                    # height: self.minimum_height  # Allow the height to grow based on content
            ...
            MDBoxLayout:
                id: input_box
                size_hint_y: None
                height: "60dp"  # Fixed height for the input box
                padding: "10dp"
                spacing: "10dp"
    ...
    MDFloatLayout:
        ...
        size_hint: None, None
        size: dp(300), dp(300)
        pos_hint: {"center_x": 0.5, "center_y": 0.5}  # Centering the wave area on the screen
        disabled: True
        opacity: 0
        ...
        WaveCircle:
            ...
            size_hint: None, None
            size: dp(240), dp(240)
            pos_hint: {"center_x": 0.5, "center_y": 0.5}  # Centering wave1
        ...
        WaveCircle:
            ...
            size_hint: None, None
            size: dp(180), dp(180)
            pos_hint: {"center_x": 0.5, "center_y": 0.5}  # Centering wave2
        ...
        WaveCircle:
            ...
            size_hint: None, None
            size: dp(120), dp(120)
            pos_hint: {"center_x": 0.5, "center_y": 0.5}  # Centering wave3
            color: 0.2, 0.6, 0.9, 1
...
class VoicePilotApp(MDApp):
    ...
    def on_send_click(self, *args):
        ...
        if user_input:
            # Disable input to prevent multiple rapid submissions
            ...
            Clock.schedule_once(lambda dt: setattr(self.root.ids.user_input, 'disabled', False), 0.1)
    ...
    def start_countdown(self):
        # Hide chat and input controls
        self.root.ids.chat_box.disabled = True
        self.root.ids.chat_box.opacity = 0
        self.hide_input_controls()
        # Show countdown label
        self.root.ids.countdown_label.disabled = False
        self.root.ids.countdown_label.opacity = 1
        self.root.ids.countdown_label.text = "5"
    ...
    def update_countdown(self, dt):
        ...
        if self.countdown_value > 0:
            self.root.ids.countdown_label.text = str(self.countdown_value)
        else:
            # Stop countdown and proceed to listening mode
            self.countdown_event.cancel()
            self.root.ids.countdown_label.disabled = True
            self.root.ids.countdown_label.opacity = 0
            self.start_listening_mode()

    def load_vosk_model(self):
        try:
            self.model = vosk.Model("./vosk-model-small-en-us-0.15")  # Specify the correct path to your Vosk model
            self.recognizer = vosk.KaldiRecognizer(self.model, 16000)
        except Exception as e:
            print(f"Error loading Vosk model: {e}")
            Clock.schedule_once(lambda dt: self.stop_listening_mode())
    ...
    def show_wave_area(self):
        ...
        self.root.ids.wave_area.disabled = False
        self.root.ids.wave_area.opacity = 1
    ...
    def hide_wave_area(self):
        ...
        self.root.ids.wave_area.disabled = True
        self.root.ids.wave_area.opacity = 0
    ...
    def add_chat_bubble(self, text, sender="user"):
        ...
        def update_layout_and_scroll(dt):
            # Set the text width and adjust heights
            ...
            self.root.ids.chat_scroll.scroll_to(bubble_row, padding=10, animate=True)

        # Schedule all updates in a single call
        Clock.schedule_once(update_layout_and_scroll, 0)
        ...
        try:
            ...
            while self.listening_mode:
                ...
                if self.recognizer.AcceptWaveform(data):
                    ...
                    if 'text' in result:
                        # Schedule UI update and stop listening on main thread
                        Clock.schedule_once(lambda dt: self._update_ui_with_result(result['text']))
                        break  # Exit loop to stop after recognizing speech
    ...
    def _update_ui_with_result(self, text):
        ...
        if text.strip():
            self.add_chat_bubble(text, sender="user")
        self.stop_listening_mode()
